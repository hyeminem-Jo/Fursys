		...
		colPosition = 0; // for printing colors on simple bar graph
 		...
 		for(var val in data){
			...
			unique = val+el.id; //unique identifier
 			...
 			out += "<div class='graphValue"+el.id+"' id='graphValue"+unique+"' style='color: red;'>"+prefix+marker+postfix+"</div>";
 		...
 		if(arr.title){
 			...
 			$('#graphHolder'+unique).prepend(arr.title).css({ 'width' : arr.width+'px', 'text-align' : 'left' });
 		}
